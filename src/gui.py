...
# x
player_x_image = pygame.image.load('./src/GUI/img/x.png')
player_x_image = pygame.transform.scale(player_x_image, (50, 50))
# y
computer_o_image = pygame.image.load('./src/GUI/img/o.png')
            ...
            for x in range(len(game_board)//3):
                for y in range(len(game_board)//3):
                    if (150 + (x*50)) <= mouse[0] <= (200 + (x*50)) and (150 + (y*50)) <= mouse[1] <= (200 + (y*50)):
                        pos = find_game_board_space(150 + (x*50), 150 + (y*50))
                        game_board[pos][0] = 'x'
                        number_of_moves += 1
